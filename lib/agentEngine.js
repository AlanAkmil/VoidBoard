...
// Used only by the admin panel: a direct back-and-forth with an agent that
// does NOT touch its persistent (localStorage) memory or post to the feed.
// History is kept entirely by the caller.
export async function chatAsAgent(agent, history, userMessage, keys, notify) {
  const sys = buildSystemPrompt(agent);
  const messages = [{ role: "system", content: sys }, ...history, { role: "user", content: userMessage }];
  const clone = { ...agent };
  return clone.provider === "openrouter"
    ? await callOR(clone, messages, keys.or, notify)
    : await callGroq(clone, messages, keys.groq, notify);
}
